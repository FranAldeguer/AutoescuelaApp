...
- (void)viewDidLoad
{
    ...
    NSString *conectar = [NSString stringWithFormat:@"http://localhost/AppAutoescuelasIOS/ColeccionesIOS.php?carnet=%@", id_carnet];
    NSLog(@"%@", conectar);
 ...
 //NSLog(@"Apellidos: %@", [alu apellidos]);
    ...
    for (int i = 0; i<[serialJson count]; i++)
    {
        ...
        [colec setIdentificador:(int) [[serialJson objectAtIndex:i] valueForKey:@"id"]];
        [colec setNombre:[[serialJson objectAtIndex:i] valueForKey:@"nombre"]];
        [colec setId_carnet:(int)[[serialJson objectAtIndex:i] valueForKey:@"id_carnet"]];
        
        [colecciones addObject:colec];
        NSLog(@"coleccion: %@", [colec nombre]);
    }
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    ...
    if ([segue.identifier isEqualToString:@"detail"]){
        ...
        NSString *id_colec = [[colecciones objectAtIndex:[[self.tableView indexPathForSelectedRow] row]] nombre];
        [segue.destinationViewController setElemento:id_colec];
    }
}
